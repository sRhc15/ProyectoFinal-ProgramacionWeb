<?php
    ...
    $message='';
    //Funcion Insertar
    function insertarN($carnet, $codigo, $zona, $final){
        global $mysqli;

        $stmt = $mysqli->prepare("INSERT INTO nota (carnet, codigo, zona, final) VALUES (?,?,?,?)");
        $stmt->bind_param('ssss', $carnet, $codigo, $zona, $final);
    ...
    if(!empty($_POST)){
        ...
        if(!empty($carnet) && !empty($codigo) && !empty($zona) && !empty($final)){
            
            $registro = insertarN($carnet, $codigo, $zona, $final);

            if($registro >0){
                echo"Nota registrada con exito!";
            }
            else{
...
<html lang="en">
    ...
    <body class="sb-nav-fixed">
        ...
        <div id="layoutSidenav">
            ...
            <!--Fin del Sidenav-->
            <!--Contenido Pagina-->
            <div id="layoutSidenav_content" class="bg-warning">
                <main>
                    <div class="container-fluid">
                        <h1 class="mt-4">Agregar Nota</h1>
                        <form method="POST" action=<?php echo $_SERVER['PHP_SELF']; ?>>
                            <div class="form-group">
                                <label class="small mb-1" for="">Estudiante</label>
                                <input class="form-control py-4" id="carnet" name="carnet" type="text" placeholder="Carnet del estudiante a asignar nota" value=""/>
                                <input type="hidden" id="id" name="id" value="" />
                            </div>
                            <div class="form-group">
                                <label class="small mb-1" for="">Curso</label>
                                <input class="form-control py-4" id="codigo" name="codigo" type="text" placeholder="Codigo del curso a asignar nota" value=""/>
                                <input type="hidden" id="id" name="id" value="" />
                            </div>
                            <div class="form-group">
                                <label class="small mb-1" for="">Zona</label>
                                <input class="form-control py-4" id="zona" name="zona" type="text" placeholder="Ingrese zona no mayor a 100" value=""/>
                                <input type="hidden" id="id" name="id" value="" />
                            </div>
                            <div class="form-group">
                                <label class="small mb-1" for="">Examen Final</label>
                                <input class="form-control py-4" id="final" name="final" type="text" placeholder="Ingrese nota del examen final no mayor a 100" value=""/>
                                <input type="hidden" id="id" name="id" value="" />
                            </div>
                            <div class="form-group mt-4 mb-0">
                                <button type="submit" class="btn btn-success">Agregar Nota</button>
                                <a href="listaN.php" class="btn btn-primary">Listado de Notas</a>
                            </div>    
                        </form>
                        <br>
                        <?php echo $message ?>
