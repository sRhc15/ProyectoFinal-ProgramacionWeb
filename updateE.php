<?php
    ...
    //Calculo de la edad con update
    $hoy =date("Y-m-d");
    $edad_diff = date_diff(date_create($fecha), date_create($hoy));
    $edad = $edad_diff->format('%y');
    
    $sql = "UPDATE estudiante SET nombre ='$nombre', fecha='$fecha', edad='$edad'  WHERE carnet = '$carnet'";
    $sql2 = "UPDATE usuario SET nombre ='$nombre' WHERE usuario ='$carnet'";
    $resultado = $mysqli->query($sql) && $mysqli->query($sql2);
?>
                ...
                <main>
                    <div class ="container">
                        <div class ="row" style="text-align: center">
                            <?php if($resultado) { ?>
                            <h3>Datos de Estudiante modificados</h3>
                            <?php } else { ?>
