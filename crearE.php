<?php
    ...
    $error='';
    //Funcion Insertar
    function insertarE($carnet, $nombre,$fecha,$edad,$contrasena,$rol){
        global $mysqli;
        
        $stmt = $mysqli->prepare("INSERT INTO estudiante (carnet,nombre,fecha,edad,contrasena) VALUES (?,?,?,?,?)");
        $stmt->bind_param('sssss', $carnet, $nombre, $fecha, $edad, $contrasena);
    ...
    if(!empty($_POST)){
        ...
        //Calculo edad (Linea 33 - 35)
        $hoy =date("Y-m-d");
        $edad_diff = date_diff(date_create($fecha), date_create($hoy));
        ...
        if(!empty($carnet) && !empty($nombre) && !empty($fecha) && !empty($contrasena)){
            //$pass_c = sha1($pass);
            $registro = insertarE($carnet, $nombre, $fecha, $edad, $contrasena,$rol);

            if($registro >0){
                $message = '<h2>Estudiante registrado con exito!</h2>'; 
            }
            else{
            ...
            <div id="layoutSidenav_content" class="bg-success">
                <main>
                    <div class="container-fluid">
                        <h1 class="mt-4">Crear Estudiante</h1>
                        <form method="POST" action=<?php echo $_SERVER['PHP_SELF']; ?>>
                            <div class="form-group">
                                <label class="small mb-1" for="">Numero de Carnet: </label>
                                <input class="form-control py-3" id="carnet" name="carnet" type="text" onKeyup="searchIdE(this.value)" placeholder="Usuario de estudiante"/>
                                <span class="lead" id="carnetMsg"></span>
                            </div>
                            <div class="form-group">
                                <label class="small mb-1" for="">Nombre Completo:</label>
                                <input class="form-control py-3" id="nombre" name="nombre" type="text" placeholder="Ingrese su Nombre" value=""/>
                                
                            </div>
                            <div class="form-group">
                                <label class="small mb-1" for="">Fecha de Nacimiento</label>
                                <input class="form-control py-3" id="fecha" name="fecha" type="text" placeholder="YYYY/MM/DD" value=""/>
                                
                            </div>
                            <div class="form-group">
                                <label class="small mb-1" for="">Contraseña</label>
                                <input class="form-control py-3" id="contrasena" name="contrasena" type="text" placeholder="Ingrese la contrasena" value=""/>
                                
                            </div>
                            <div class="form-group mt-4 mb-0">
                                <input type="hidden" value="11" name="type" id="type">
                                <button type="submit" class="btn btn-primary">Agregar Estudiante</button>
                                <!--<input type="button" class="btn btn-primary" id="sendEData" data-toggle="modal" data-target="#messageModal" onClick="sendDataE()" value="Agregar estudiante">-->
                                <a href="listaE.php" class="btn btn-secondary">Listado Estudiantes</a>
                            </div>    
                        </form>
                        <br>
                        <?php echo $message ?>
