<?php
    ...
    $message='';
    //Funcion Insertar
    function insertarC($codigo, $nombre,$desc){
        global $mysqli;

        $stmt = $mysqli->prepare("INSERT INTO curso (codigo,nombre,descripcion) VALUES (?,?,?)");
    ...
    if(!empty($_POST)){
        ...
        if(!empty($codigo) && !empty($nombre) && !empty($desc)){
            //$pass_c = sha1($pass);
            $registro = insertarC($codigo, $nombre, $desc);

            if($registro >0){
                $message = '<h2>Curso creado con exito!</h2>'; 
            }
            else{
                $message = '<h2>Curso Creado</h2>'; //Debe ir mensaje de error
            }
        }
        else{
            $message = "<h2>Hubo un error al crear curso, revise que los datos sean correctos</h2>";
        }   
                    ...
                    <div class="container-fluid">
                        ...
                        <form method="POST" action=<?php echo $_SERVER['PHP_SELF']; ?>>
                            <div class="form-group">
                                <label class="small mb-1" for="">Codigo</label>
                                <input class="form-control py-4" id="codigo" name="codigo" type="text" placeholder="Codigo Numerico" value=""/>
                                <input type="hidden" id="id" name="id" value="" />
                            </div>
                            <div class="form-group">
                                <label class="small mb-1" for="">Nombre</label>
                                <input class="form-control py-4" id="nombre" name="nombre" type="text" placeholder="Ingrese nombre del curso" value=""/>
                                <input type="hidden" id="id" name="id" value="" />
                            </div>
                            <div class="form-group">
                                <label class="small mb-1" for="">Descripcion</label>
                                <input class="form-control py-4" id="desc" name="desc" type="text" placeholder="Ingrese la descripcion del curso" value=""/>
                                <input type="hidden" id="id" name="id" value="" />
                            </div>
                            <div class="form-group mt-4 mb-0">
                                <button type="submit" class="btn btn-success">Crear curso</button>
                                <a href="listaC.php" class="btn btn-secondary">Listado cursos</a>
                            </div>    
                        </form>
                        <br>
                        <?php echo $message ?>
