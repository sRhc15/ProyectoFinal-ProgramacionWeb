<?php
    ...
    $error='';
    //Funcion Insertar
    function insertarP($usuario, $contrasena,$nombre,$telefono, $correo, $rol){
        global $mysqli;
        
        $stmt = $mysqli->prepare("INSERT INTO padre (usuario,contrasena,nombre,telefono,correo) VALUES (?,?,?,?,?)");
        $stmt->bind_param('sssss', $usuario, $contrasena, $nombre,$telefono,$correo);

        $stmt2 = $mysqli->prepare("INSERT INTO usuario (usuario,contrasena,nombre,rol) VALUES (?,?,?,?)");
        $stmt2->bind_param('ssss', $usuario, $contrasena, $nombre,$rol);
    ...
    if(!empty($_POST)){
        ...
        if(!empty($usuario) && !empty($contrasena) && !empty($nombre) && !empty($telefono) && !empty($correo)){
            
            $registro = insertarp($usuario, $contrasena, $nombre, $telefono, $correo, $rol);

            if($registro >0){
                $message = '<h2>Padre registrado con exito!</h2>'; 
            }
            else{
            ...
            <div id="layoutSidenav_content" class="bg-info">
                <main>
                    <div class="container-fluid">
                        <h1 class="mt-4">Crear Padre</h1>
                        <form method="POST" action=<?php echo $_SERVER['PHP_SELF'] ?>>
                            <div class="form-group">
                                <label class="small mb-1" for="">Usuario</label>
                                <input class="form-control py-4" id="usuario" name="usuario" type="text" placeholder="Ingrese usuario" value=""/>
                                <input type="hidden" id="" name="" value="" />
                            </div>
                            <div class="form-group">
                                <label class="small mb-1" for="">Contrasena</label>
                                <input class="form-control py-4" id="contrasena" name="contrasena" type="text" placeholder="Ingrese nombre" value=""/>
                                <input type="hidden" id="" name="" value="" />
                            </div>
                            <div class="form-group">
                                <label class="small mb-1" for="">Nombre Completo</label>
                                <input class="form-control py-4" id="nombre" name="nombre" type="text" placeholder="Ingrese nombre" value=""/>
                                <input type="hidden" id="" name="" value="" />
                            </div>
                            <div class="form-group">
                                <label class="small mb-1" for="">Numero telefonico</label>
                                <input class="form-control py-4" id="telefono" name="telefono" type="text" placeholder="Ingrese No. de telefono" value=""/>
                                <input type="hidden" id="" name="" value="" />
                            </div>
                            <div class="form-group">
                                <label class="small mb-1" for="">Correo Electronico</label>
                                <input class="form-control py-4" id="correo" name="correo" type="text" placeholder="Ingrese Email" value=""/>
                                <input type="hidden" id="" name="" value="" />
                            </div>
                            <div class="form-group mt-4 mb-0">
                                <button type="submit" class="btn btn-success">Agregar Padre</button>
                                <a href="listaP.php" class="btn btn-primary">Listado de Padres</a>
                            </div>    
                        </form>
                        <br>
                        <?php echo $message ?>
